defmodule KempelenWeb.Graphql.Resolvers.OrganizationMemberships do
  def list(_parent, _arguments, _resolution) do
    {:ok, Kempelen.Database.Repo.all(Kempelen.Models.OrganizationMembership)}
  end

  def fetch(_parent, %{id: id}, _resolution) when not is_nil(id) do
    {:ok, Kempelen.Database.Repo.get(Kempelen.Models.OrganizationMembership, id)}
  end
  ...
  def create(_parent, arguments, _resolution) do
    ...
    %Kempelen.Models.OrganizationMembership{}
      |> Kempelen.Models.OrganizationMembership.changeset(attributes)
      |> case do
  ...
  def update(_parent, %{id: id} = arguments, _resolution) when not is_nil(id) do
    Kempelen.Database.Repo.get(Kempelen.Models.OrganizationMembership, id)
      |> Kempelen.Models.OrganizationMembership.changeset(arguments)
      |> case do
        %Ecto.Changeset{valid?: true} = changeset -> Kempelen.Database.Repo.insert(changeset)
        %Ecto.Changeset{valid?: false} = changeset -> {:error, changeset}
      end
